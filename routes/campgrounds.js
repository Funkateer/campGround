...
//INDEX - show all campgrounds ..  we took out the campground route and put the "/"
router.get("/", function(req, res){
	// passport makes a lil note of the user you can access by using so you can use it to know if somebody is logged or not to customize our nav bar lol
	...
	Campground.find({}, function(err, allCampgrounds){
		if(err){
			...
			res.render("campgrounds/index",{campgrounds:allCampgrounds, currentUser: req.user}); // here we put the note about the user.. and you need to put them in all routes there is a function on top for that
		}
	});
});
...
router.post("/", middleware.isLoggedIn, function(req, res){
	...
	var newCampground = {name: name, image: image, description: desc, price: price, author:author}; // we add the author to the new campground
